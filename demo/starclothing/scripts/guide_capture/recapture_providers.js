...
const OUT = '/root/ai_infra/demo/starclothing/shots/mgmt/providers.png';
...
(async () => {
  ...
  await page.goto(`${WEB}/providers`, { waitUntil: 'domcontentloaded', timeout: 30000 });
  ...
  for (let i = 0; i < n; i++) {
    try {
      ...
      const hasProv = await page.getByText(/aliyun|aliyuncs|maas/, { exact: false }).first().isVisible({ timeout: 800 }).catch(() => false);
      console.log(`  click #${i}: hasProv=${hasProv}`);
      if (hasProv) { ok = true; break; }
  ...
  console.log(ok ? 'OK providers captured' : 'WARN keys may still be empty -> ' + OUT);
  await browser.close();
