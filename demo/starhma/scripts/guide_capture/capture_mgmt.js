// starhma 管理端截图（admin 登录 → 16 个功能页 → 左树点选有数据的归口节点 → 截图）
// admin 是 org_admin（绑 organization_id）→ OrgSelect 自动锁星途热熔胶组织，无需手点；只点左树数据节点。
//   keys / providers / dlp / tools-data-interfaces / tools-ontology / agent-workspaces → 星途热熔胶 (org level)
//   agent-rag / tools-skills / agent-agents → 研发中心 (dept, 设计规范RAG + design skill + des agent)
//   agent-memory → 配方研发工程师 (des-engineer user, 有记忆)
...
const SLUG = 'starhma', USER = 'admin', PASS = '12345678';
const ORG = '星途热熔胶';
const OUT = '/root/ai_infra/demo/starhma/shots/mgmt';
...
const PAGES = [
  ...
  ['/agent/agents',         'agent-agents',         '研发中心'],
  ['/agent/rag',            'agent-rag',            '研发中心'],
  ['/agent/memory',        'agent-memory',         '配方研发工程师'],
  ['/tools/data-interfaces','tools-data-interfaces', ORG],
  ['/tools/skills',         'tools-skills',         '研发中心'],
  ...
  for (const [route, file, nodeText] of PAGES) {
    try {
      ...
      if (route === '/org/structure') {
        ...
        await page.getByText('研发中心', { exact: true }).first().click({ timeout: 10000 });
      ...
      console.log(`OK ${file} (${route}) node=${nodeText || (route === '/org/structure' ? '组织→研发中心' : (route === '/monitor/router' ? '路由指标tab' : '-'))}`);
    } catch (e) {
