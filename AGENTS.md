...
| 旧音频接口禁止推理字段外泄、缺失租户/主体时拒绝工作空间访问 | 本次候选分支已修改，未部署 | 聚焦单元测试通过；数据库和真实角色回归待完成 |
| 公共工具与业务候选统一相关性排序 | 本次候选分支已修改，未部署 | 聚焦单元测试通过；真实任务回归待完成 |
| 普通录音回填、点击朗读、沉浸式轮流语音 | 本轮后续实施 | 尚未完成，不得声称上线 |
