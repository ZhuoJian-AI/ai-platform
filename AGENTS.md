...
- 本批代码完成、待发布：普通业务调用按同 Task 历史引用匹配未过期的待确认请求，参数、页面、版本相同才复用原卡片；执行仍实时校验权限。不匹配已结束记录，不能代替完整逻辑操作关联。143 项聚焦测试通过，详见 `docs/handoff/2026-09-13_pending-action-dedup.md`。
