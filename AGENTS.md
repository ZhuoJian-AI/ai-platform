...
- 候选代码已实现：普通业务写入按同 Task 持久回执匹配原参数、页面和版本，不依赖主脑短历史窗口；同轮新工具调用 ID 不产生重复写入。明确再次执行使用独立工具和“再次执行”确认卡片。
- 候选代码已实现：按原请求分别记录完成、待确认、执行中、未知与未完成；文件交付单独记账，恢复时保留原成功来源。账本记录已经尝试的步骤，不假装能够证明模型从未调用过的目标已经完成。
- 候选代码已实现：长回复/表格按需生成语义口播，复用管理员主脑部署；普通文字聊天不触发摘要模型。分句流保留最后一个片段给最终结果摘要，失败保留正文，不重新执行业务操作。
...
- 当前发布状态：本批尚未部署；发布提交、digest 和浏览器结果以 `docs/handoff/2026-09-13_recovery-speech-completion.md` 为准。下列历史待办中对应代码项已由本批候选覆盖，不能据此提前声称线上生效。
...
- [ ] 模型另行创建新业务调用时的跨轮逻辑操作关联与重复写入保护；不能只依靠模型选择恢复工具。
...
- [ ] 完整多目标恢复：分别保留修改、分析、文件交付状态，仅重跑未完成步骤。
