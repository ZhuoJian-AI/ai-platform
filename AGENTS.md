...
每次交付回复必须附本文件的可点击链接。已实现、已部署、实测通过分别记录，不将历史待办或局部测试混成整体完成。

### 合并批次：恢复与语音摘要（2026-09-13）

- 已实现并部署：普通业务写入按同 Task 持久回执匹配原参数、页面和版本，不依赖主脑短历史窗口；同轮新工具调用 ID 不产生重复写入。明确再次执行使用独立工具和“再次执行”确认卡片。
- 已实现并部署：按原请求分别记录完成、待确认、执行中、未知与未完成；文件交付单独记账，恢复时保留原成功来源。账本记录已经尝试的步骤，不假装能够证明模型从未调用过的目标已经完成。
- 已实现并部署：长回复/表格按需生成语义口播，复用管理员主脑部署；普通文字聊天不触发摘要模型。分句流保留最后一个片段给最终结果摘要，失败保留正文，不重新执行业务操作。
- 验证：169 项聚焦测试通过；独立 PostgreSQL 的同 Task 持久引用、参数差异及其他 Task/员工隔离实测通过；zhangsan 当前角色与真实配置模型的摘要服务验证通过，正确保留“业务成功、文件失败”。该服务验证不是浏览器完整业务恢复验收。
- 当前发布状态：source `5ca01e7`、manifest `b751c05`，部署 `voicefix6dc92e4a3def0a8e` finished；运行 digest 已核对，9 服务 healthy，公开 health 200。部署后 Playwright 连接返回 `Transport closed`，本批浏览器回归尚未通过，不能用部署健康替代。详见 `docs/handoff/2026-09-13_recovery-speech-completion.md`。
- 仍需据实保留：旧记录丢失参数不能补造绑定；历史 401 缺证据；chouchou 缺真实会话；204A231 图片依赖子系统接口。完整自然语言多目标恢复与跨角色浏览器验收不以单元测试替代。

- 已部署：统一 Task/视图接续、导航一次定位、语音模式与分句播报、未知写入人工核实、部分结果提示、历史执行引用、未清理请求的参数绑定。
- 本批已部署（2026-09-13）：已结束请求以加密摘要保留参数绑定，清理完整业务参数；source `695ca9c`、manifest `c1bfef1`，119 项聚焦测试通过，9 服务健康，root 现有会话刷新 200。此证据不等于新增员工业务实测通过。
- 本批已部署（2026-09-13）：同 Task 按原请求引用恢复真实回执或原确认卡片，恢复工具不提交远端写入；source `4887ed8`、manifest `b1fc8ac`、Coolify `voicefix7e3b4b26c7370c81` finished。136 项聚焦检查通过，9 服务健康，health、root 页面及 auth/me 均 200；不代表真人多步骤恢复验收通过。
- [x] 同 Task 普通写入按持久引用自动复用原回执，并保留显式再次执行确认；代码与部署完成，浏览器闭环待验收。
- 本批已部署（2026-09-13）：普通业务调用按同 Task 历史引用匹配未过期的待确认请求，参数、页面、版本相同才复用原卡片；执行仍实时校验权限。不匹配已结束记录，不能代替完整逻辑操作关联。source `566b70d`、manifest `df6c400`、Coolify `voicefix0250bc2e0ddb058e` finished；143 项聚焦测试通过，9 服务健康，health/root 页面/auth/me 均 200。未新增真人业务恢复验收，详见 `docs/handoff/2026-09-13_pending-action-dedup.md`。
- [x] 已尝试步骤的修改、分析、文件交付状态分别持久化；普通写入恢复不重复提交成功操作。完整自然语言多目标浏览器验收仍在下方待办中。
- 本批已部署（2026-09-13）：业务修改仅将明确 `completed` 回执计作成功；缺少状态、unknown/executing/expired/rejected 不再因不在失败列表而误记成功。source `154f03d`、manifest `08b0fe1`、Coolify `voicefix65c287ceb626a836` finished。30 项针对性测试通过，包含错误后成功恢复；9 服务健康，health/root 页面/auth/me 200。详见 `docs/handoff/2026-09-13_explicit-mutation-completion.md`。
- 本批已部署（2026-09-13）：恢复已完成业务回执后，将原来源、执行时间和快照交给后续 Artifact 生成；不把恢复时间当作执行时间，失败/未知/待确认回执不写入成功来源。source `964ea67`、manifest `c1b21fa`，Coolify `voicefix5084fece6af836c8` finished；30 项聚焦测试通过，9 服务健康，health/root 页面/auth/me 200。尚无真实文件恢复验收；见 `docs/handoff/2026-09-13_recovered-artifact-provenance.md`。
- [ ] 旧请求若参数早已清空，无法补造摘要；完成有明确证据的历史兼容处理。
- [x] 长回复语义口播代码与部署完成，真实模型摘要验证通过；最终页面播放待验收。
- [ ] 历史员工 401：取得原始请求、会话续期与 auth_epoch 证据后处理，现阶段仍待查。
- [ ] chouchou 真实会话专项验收；204A231 图片交付依赖子系统真实图片接口，数量统计不算通过。
- [ ] 新增恢复链路最终完成后，做一次聚焦管理员/员工业务闭环验收，不重复全部 CRUD。

本批细节：`docs/handoff/2026-09-13_action-recovery-reference.md`。以下按日期保留历史证据，不表示旧待办仍全部未完成。
...
2026-09-12 恢复上下文证据已部署：后端 source `6b539e3`，manifest `55db9ad`，Coolify `voicefixa0a2e35de8bda7c7`。新运行保留 Action 请求引用及未知结果标记，页面业务历史区分已完成、待确认、执行中、未知和未核实，不以 ok=true 代替完成回执。43 项聚焦测试通过，9 服务健康，线上 root 会话刷新 200。总入口对业务历史引用的接续以及完整跨轮去重仍待完成；详见 `docs/handoff/2026-09-12_recovery-history.md`。
...
2026-09-12 语音体验后续批次：输入框旁“语音模式”、新会话首轮、同 Task 视图共享控制器与按轮一次定位已部署，source `7d93aba`，manifest `3310ac6`。类型检查、构建、候选滚动测试和线上真实员工开麦退出通过；首句真实模型与跨页语音操作未专项验收。分句流式播报与未知写入管理核实仍未完成。用户已确认上一发布的基础语音对话实测通过，该反馈不等于真实业务修改语音闭环验收。

聊天在进入本轮时定位一次，随后不得随 token 或媒体加载强制滚动；用户通过“回到当前回复”自行定位。语音模式与普通录音不同：前者自动提交，后者转写回填后由用户编辑发送。新会话在首次有效转写后建立 Task；仅开麦或静默不建立 Task。

2026-09-12 简化控制已部署：source `1b646f4`，manifest `3933010`。一键“语音模式”，常驻状态、暂停/继续及退出；静音和结束本句位于次级设置；普通录音改名“语音输入”。线上员工开麦/退出聚焦回归通过，9 服务健康。此批不包含分句流式 TTS。

分句播报通过 PR #138 接通公开文字实时转发、Run 级持久语音片段、标准 TTS job 与有序播放，并已按上述最新发布上线。真实 PostgreSQL 独立连接验收通过；隔离候选真实主脑、worker、MiMo TTS 和浏览器播放：首声 33.8 秒，最终事件 88.9 秒，29 段播放。该时序测试直接调用浏览器 adapter，不冒充真人麦克风实测，也不承诺语音与文字同时结束。详见 `docs/handoff/2026-09-12_voice-stream-framing.md`。
...
| 未知写入受控核实 | 已合并并部署 staging，source `28dcfbd`，manifest `a8cdf4c` | 候选数据库并发/审计与真实管理员提交闭环通过；线上只读入口和接口通过，未在 staging 制造未知业务写入 |
| 多目标部分成功恢复、历史运行 401 排查 | 尚未完成 | 未取得真实结果或请求证据前不得声称解决 |
