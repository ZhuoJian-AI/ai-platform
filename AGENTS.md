...
分句播报候选分支 `feat/voice-sentence-stream-20260912` 已接通公开文字实时转发、Run 级持久语音片段、标准 TTS job 与有序播放。真实 PostgreSQL 独立连接验收通过；隔离候选环境真实登录、主脑、独立 worker、MiMo TTS 和浏览器音频播放通过：首声 33.8 秒，最终事件 88.9 秒，29 段播放。该测试直接调用真实浏览器 adapter，不替代三个界面入口、跨页和真人麦克风验收。未部署，不能宣称线上已支持边生成边播报。详见 `docs/handoff/2026-09-12_voice-stream-framing.md`。
