...
- 本批代码完成、待部署：恢复已完成业务回执后，将原来源、执行时间和快照交给后续 Artifact 生成；不把恢复时间当作执行时间，失败/未知/待确认回执不写入成功来源。30 项聚焦测试通过，尚无真实文件恢复验收；见 `docs/handoff/2026-09-13_recovered-artifact-provenance.md`。
