...
- `_synthesize` 原先仅在供应商调用前校验角色及目标空间。现在供应商返回后、任何文件写入前，再经 `_resolve_tool_workspace` 回调刷新员工角色，核对仍是相同员工和目标空间，并复查语音权限。
- 工作空间撤权、语音撤权或目标变化时返回不可重试中文失败，不调用 ingest，不生成 Artifact。
