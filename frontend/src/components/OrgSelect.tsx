...
export default function OrgSelect({ value, onChange }: { value?: string; onChange: (id: string) => void }) {
  ...
  // 平台模式：受控方未指定 value 时，回填默认组织，触发下游查询
  useEffect(() => {
    if (!isOrgScoped() && !value && defaultOrgId) onChange(defaultOrgId);
    // 仅依赖 defaultOrgId：orgs 加载完成后回填一次即可
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [defaultOrgId, isOrgScoped]);
