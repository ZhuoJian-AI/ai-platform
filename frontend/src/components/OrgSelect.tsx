import { useCallback, useEffect } from 'react';
import { useQuery, useQueryClient } from '@tanstack/react-query';
import { Select, Tag } from 'antd';
import { organizations } from '../api/client';
import { useAuth } from '../context/AuthContext';
import { rotateAdminOrganizationScope } from '../auth/adminSession';

const SELECTED_ORG_STORAGE_KEY = 'zhuojian:selected-organization-id';

/** 组织选择器：缓存组织列表，受控返回当前 orgId。

未传 value 时自动选用平台默认组织（is_default=True），
使各管理页面进入即「优先显示默认组织」，同时仍允许手动切换。

企业管理员（enterprise_admin / 绑定 organization_id）进入「受限模式」：
强制锁定到自己被指派的组织、不可切换，并显示组织名而非下拉列表。
这样所有调用方无需逐个改造即可满足「去掉组织下拉、只看自己组织」的需求。
*/
export default function OrgSelect({ value, onChange }: { value?: string; onChange: (id: string) => void }) {
  const queryClient = useQueryClient();
  const { isOrgScoped, admin } = useAuth();
  const scopedOrgId = admin?.organization_id ?? undefined;

  const { data: orgs } = useQuery({
    queryKey: ['orgs'],
    queryFn: organizations.list,
    enabled: !isOrgScoped(),
  });

  const changeOrganization = useCallback((id: string, remember: boolean) => {
    if (!id || id === value) return;
    // React Query cancellation alone is insufficient because most historical
    // query functions did not forward its AbortSignal. Rotate the transport
    // scope first, then remove every identity/tenant-derived cache entry.
    rotateAdminOrganizationScope();
    void queryClient.cancelQueries();
    queryClient.clear();
    if (remember) window.sessionStorage.setItem(SELECTED_ORG_STORAGE_KEY, id);
    else window.sessionStorage.removeItem(SELECTED_ORG_STORAGE_KEY);
    window.dispatchEvent(new CustomEvent('zhuojian:organization-changed', { detail: { organizationId: id } }));
    onChange(id);
  }, [onChange, queryClient, value]);

  // 默认组织 id；无显式默认时回退到首个组织
  const defaultOrgId = orgs?.find((o) => o.is_default)?.id ?? orgs?.[0]?.id;
  const storedOrgId = typeof window === 'undefined'
    ? undefined
    : window.sessionStorage.getItem(SELECTED_ORG_STORAGE_KEY) ?? undefined;
  const rememberedOrgId = storedOrgId && orgs?.some((org) => org.id === storedOrgId)
    ? storedOrgId
    : undefined;

  // 受限模式：强制锁定到自己的组织
  useEffect(() => {
    if (isOrgScoped() && scopedOrgId && value !== scopedOrgId) {
      changeOrganization(scopedOrgId, false);
    }
  }, [changeOrganization, isOrgScoped, scopedOrgId, value]);

  // 平台模式：跨页面保留平台管理员正在管理的企业；没有记忆时才使用默认组织。
  useEffect(() => {
    const preferredOrgId = rememberedOrgId ?? defaultOrgId;
    if (!isOrgScoped() && !value && preferredOrgId) changeOrganization(preferredOrgId, true);
    // 组织列表加载完成后回填一次；value 由父页面继续作为唯一受控状态。
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [changeOrganization, defaultOrgId, rememberedOrgId, isOrgScoped, value]);

  // 受限模式：渲染为不可点的组织名标签（去掉下拉）
  if (isOrgScoped()) {
    return (
      <Tag color="blue" style={{ marginInlineEnd: 0 }}>
        {admin?.organization_name || orgs?.find((o) => o.id === scopedOrgId)?.name || '当前组织'}
      </Tag>
    );
  }

  return (
    <Select
      placeholder="选择组织"
      style={{ width: 240 }}
      value={value ?? rememberedOrgId ?? defaultOrgId}
      onChange={(id) => changeOrganization(id, true)}
      options={orgs?.map((o) => ({ value: o.id, label: o.is_default ? `${o.name}（默认）` : o.name })) ?? []}
      notFoundContent="请先创建组织"
    />
  );
}
