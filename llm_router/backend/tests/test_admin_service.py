...
async def test_ensure_super_admin_tolerates_multiple_active_admins(db_session):
    first = Admin(username="root-a", password_hash="x", role="super_admin", is_active=True)
    second = Admin(username="root-b", password_hash="x", role="super_admin", is_active=True)
