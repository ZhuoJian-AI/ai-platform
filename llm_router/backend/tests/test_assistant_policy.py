...
def _craft_state(**overrides) -> dict:
    state = {
        ...
        "messages": [{"role": "user", "content": "记住我偏好账期长的供应商"}],
...
async def test_craft_turn_offers_memory_tools_to_a_terminal_user(monkeypatch):
    principal = SimpleNamespace(id="user-1", organization_id=uuid4(), department_id=None, role="member")
...
async def test_execute_tool_call_dispatches_memory_tools_for_the_current_principal(monkeypatch):
    ...
    message, preview, ok = await nodes._execute_tool_call(
        state,
        {"id": "c1", "name": "write_memory", "arguments": {"content": "供应商 A → 账期 → 60 天"}},
        registry,
    )
    assert ok is True
    ...
    message, _preview, ok = await nodes._execute_tool_call(
        state,
        {"id": "c2", "name": "read_memory", "arguments": "{}"},
        registry,
    )
    assert ok is True
    ...
    message, _preview, ok = await nodes._execute_tool_call(
        state,
        {"id": "c3", "name": "write_memory", "arguments": {"content": "   "}},
        registry,
    )
    assert ok is False
