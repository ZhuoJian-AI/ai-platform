...
def _build_read(api_key) -> dict:
    """Serialize API-key metadata without ever recovering bearer plaintext."""
    return _ApiKeyReadInner.model_validate(api_key).model_dump()
...
async def create_org_key(
    ...
    api_key, full_key = await create_api_key(db, org_id, data)
...
async def create_dept_key(
    ...
    api_key, full_key = await create_api_key(db, dept_id=dept_id, org_id=dept.organization_id, data=data)
    return ApiKeyCreateResponse(**_build_read(api_key), key=full_key)
