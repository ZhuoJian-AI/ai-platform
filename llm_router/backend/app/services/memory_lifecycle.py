"""Memory lifecycle — keep each organization/department node paired with one memory.

镜像 ``workspace_lifecycle``：每个组织 / 部门节点恰好对应一条长期记忆，绑定键为
``(organization_id, scope_type, scope_id)``。节点增删改时同步——创建 / 重命名时写入并刷新
``## 档案`` 分节（组织写组织名；部门写组织 + 部门名，均 markdown），
...
def _build_archive_section(org_name: str | None, dept_name: str | None) -> str:
    """构建「## 档案」分节：按层级写入所属名称，无任何名称时返回空串。"""
    lines: list[str] = []
    if org_name:
...
async def ensure_node_memory(
    db: AsyncSession, org_id: UUID, scope_type: str, scope_id: str | None, *,
    org_name: str | None, dept_name: str | None = None,
) -> Memory:
    """确保组织或部门节点存在绑定记忆，并刷新其 ``## 档案`` 分节。
    ...
    section = _build_archive_section(org_name, dept_name)
